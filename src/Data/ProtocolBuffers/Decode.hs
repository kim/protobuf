...
fieldDecode
  :: forall a b i n p . (DecodeWire a, Monoid a, SingI n)
  => (a -> b)
...
instance (DecodeWire a, SingI n) => GDecode (K1 i (Field n (OptionalField (Last (Value a))))) where
  gdecode msg = fieldDecode Optional msg <|> pure (K1 mempty)

instance (Enum a, SingI n) => GDecode (K1 i (Field n (RequiredField (Always (Enumeration a))))) where
  gdecode msg = do
...
instance (Enum a, SingI n) => GDecode (K1 i (Field n (OptionalField (Last (Enumeration a))))) where
  gdecode msg = do
...
instance (DecodeWire a, SingI n) => GDecode (K1 i (Field n (RequiredField (Always (Value a))))) where
